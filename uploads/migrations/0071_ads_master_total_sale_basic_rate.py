# Adds a `total_sale_basic_rate` column to the per-platform ads master views
# (Blinkit / Swiggy / Zepto / BigBasket / Flipkart — NOT Amazon).
#
# Formula: basic_rate * direct_qty_sold, where basic_rate is looked up from
# monthly_landing_rate by matching the ads SKU code against sku_code, the
# platform (normalised format), and the reporting month. A correlated scalar
# subquery (LIMIT 1, latest created_at) is used instead of a JOIN so it can
# never fan out the master rows even if a landing-rate key were duplicated.
#
# Flipkart's ads feed is campaign-level (no SKU code, no per-SKU qty), so its
# column is created as NULL for uniformity — it cannot be computed.
#
# CREATE OR REPLACE only appends the trailing column, so existing consumers of
# these views keep working unchanged. No view depends on these five, so the
# reverse path safely drops + recreates the originals.
from django.db import migrations


# ── Basic-rate scalar subquery, parameterised per platform. ──────────────────
...
BLINKIT_QTY = "direct_qty_sold"
SWIGGY_QTY = "direct_qty_sold"
...
CREATE OR REPLACE VIEW blinkit_ads_master AS
 SELECT date, campaign_id, campaign_name, direct_qty_sold, indirect_qty_sold,
        ...
        {_basic_rate('format_sku_code', 'date', 'BLINKIT')} * COALESCE({BLINKIT_QTY}, 0)
...
CREATE OR REPLACE VIEW swiggy_ads_master AS
 SELECT date, campaign_id, campaign_name, direct_qty_sold, impressions, ad_spent,
        ...
        {_basic_rate('format_sku_code', 'date', 'SWIGGY')} * COALESCE({SWIGGY_QTY}, 0)
            AS total_sale_basic_rate
...
# ── Reverse: drop + recreate the original view definitions (no trailing col). ─
REVERSE = """
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("uploads", "0070_amazon_sec_city_rename_dedupe"),
    ]
