...
# Adds `total_distributor_commission` to amazon_sec_range_master_view. The
# Primary master_po view already exposes this column directly; the Amazon
# Secondary view has no commission ledger, so it is derived as the margin the
# distributor keeps: calculated_shipped_revenue - shipped_revenue_after_margin.
# CREATE OR REPLACE VIEW only allows appending new columns at the END of the
# select list, so the new column is added last (after margin_category) and all
# existing columns keep their order. Everything else mirrors migration 0019.
FORWARD_SQL = """
...
SELECT
    ...
    margin_category,
    COALESCE(calculated_shipped_revenue, 0)
        - COALESCE(shipped_revenue_after_margin, 0) AS total_distributor_commission
FROM final_calc;
"""


# Restores the migration 0019 definition (without total_distributor_commission).
REVERSE_SQL = """
CREATE OR REPLACE VIEW amazon_sec_range_master_view AS
WITH master_lookup AS (
    SELECT DISTINCT ON (format_sku_code)
...
class Migration(migrations.Migration):
    dependencies = [
        ("platforms", "0047_flipkart_state_sales_master_is_litre"),
    ]
