...
FORWARD_SQL = """
CREATE TABLE IF NOT EXISTS amazon_sec_range_margins (
    id BIGSERIAL PRIMARY KEY,
    asin TEXT NOT NULL,
    margin_category TEXT,
    margin_pct NUMERIC(10, 4),
    source_file TEXT,
    source_sheet TEXT NOT NULL DEFAULT 'MARGINS',
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT amazon_sec_range_margins_asin_key UNIQUE (asin)
);

CREATE INDEX IF NOT EXISTS idx_amazon_sec_range_margins_asin
    ON amazon_sec_range_margins (asin);

CREATE OR REPLACE FUNCTION set_amazon_sec_range_margins_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
DROP TRIGGER IF EXISTS amazon_sec_range_margins_set_updated_at
    ON amazon_sec_range_margins;

CREATE TRIGGER amazon_sec_range_margins_set_updated_at
BEFORE UPDATE ON amazon_sec_range_margins
FOR EACH ROW
EXECUTE FUNCTION set_amazon_sec_range_margins_updated_at();
...
REVERSE_SQL = """
DROP VIEW IF EXISTS amazon_sec_range_master_view;
DROP TRIGGER IF EXISTS amazon_sec_range_margins_set_updated_at
    ON amazon_sec_range_margins;
DROP FUNCTION IF EXISTS set_amazon_sec_range_margins_updated_at();
DROP INDEX IF EXISTS idx_amazon_sec_range_margins_asin;
DROP TABLE IF EXISTS amazon_sec_range_margins;
"""


class Migration(migrations.Migration):
    dependencies = [
        ("platforms", "0017_amazon_price_data_updated_at"),
    ]
